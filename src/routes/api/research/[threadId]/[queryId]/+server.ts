...
import { threads, queries, modelGroups } from '../../../../../db/schema';
import { researchFlow } from '$lib/ai/research';
		...
		new ReadableStream({
			async start(controller) {
				...
				try {
					...
					const result = researchFlow.stream({
					...
					controller.enqueue(
						`data: ${JSON.stringify({ type: 'complete', content: combined })}\n\n`
					);
				} catch (error) {
